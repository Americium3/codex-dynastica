...
> ## ⚠️ 项目已放弃 — 2026-05-23
>
> 本项目**不再继续开发**，GitHub 仓库已**归档**（只读）。
>
> Phase 0 至 Phase 1.2 已落地（端到端流水线、王室图书馆游戏内 UI、`emit-loc` CLI、`vox-companion` 存档监听托盘程序——归档时 104 条测试全绿），但项目在云端 API 选择器、敌国/教会视角、历史漂移等阶段之前停摆。
>
> 代码留在线上作为参考 / fork 素材；不再受理 issue 与 pull request。如有意继续，请基于 MIT license 自由 fork —— 请勿在本仓库开 issue。
